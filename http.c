...
http_response_init(struct worker *w, int code, const char *msg) {
	...
	http_response_set_header(r, "Server", "STKZ");
...
http_response_set_header(r, "Access-Control-Allow-Headers", "X-Requested-With, Content-Type, Authorization,accept, accept-charset, accept-encoding, accept-language, authorization, content-length, content-type, host, origin, proxy-connection, referer, user-agent, x-requested-with");
	return r;
}
